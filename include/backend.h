...
#define HOLD(x, b)                                                    \
  ((--be_freehold)                                                    \
       ? (((void)(*be_holdbits++ = b)), ((void)(*be_holdSMVs++ = x))) \
       : behold(x, b))
...
#define EMIT8(n)                              \
  {                                           \
    if (emit_section->left < 1) be_extend(1); \
    *emit_section->ptr++ = (n);               \
    emit_section->left--;                     \
    (*current_loc)++;                         \
  }
#define EMIT16(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 2) be_extend(2); \
    *emit_section->ptr++ = _ >> 8;            \
    *emit_section->ptr++ = _;                 \
    emit_section->left -= 2;                  \
    (*current_loc) += 2;                      \
  }
#define EMIT24(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 3) be_extend(3); \
    *emit_section->ptr++ = _ >> 16;           \
    *emit_section->ptr++ = _ >> 8;            \
    *emit_section->ptr++ = _;                 \
    emit_section->left -= 3;                  \
    (*current_loc) += 3;                      \
...
#define EMIT32(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 4) be_extend(4); \
    *emit_section->ptr++ = _ >> 24;           \
    *emit_section->ptr++ = _ >> 16;           \
    *emit_section->ptr++ = _ >> 8;            \
    *emit_section->ptr++ = _;                 \
    emit_section->left -= 4;                  \
...
#define EMIT40(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 5) be_extend(5); \
    *emit_section->ptr++ = _ >> 32;           \
    *emit_section->ptr++ = _ >> 24;           \
    *emit_section->ptr++ = _ >> 16;           \
    *emit_section->ptr++ = _ >> 8;            \
    *emit_section->ptr++ = _;                 \
...
#define EMIT48(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 6) be_extend(6); \
    *emit_section->ptr++ = _ >> 40;           \
    *emit_section->ptr++ = _ >> 32;           \
    *emit_section->ptr++ = _ >> 24;           \
    *emit_section->ptr++ = _ >> 16;           \
    *emit_section->ptr++ = _ >> 8;            \
...
#define EMIT56(n)                             \
  {                                           \
    numtype _ = (n);                          \
    if (emit_section->left < 7) be_extend(7); \
    *emit_section->ptr++ = _ >> 48;           \
    *emit_section->ptr++ = _ >> 40;           \
    *emit_section->ptr++ = _ >> 32;           \
    *emit_section->ptr++ = _ >> 24;           \
    *emit_section->ptr++ = _ >> 16;           \
