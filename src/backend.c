...
void be_extend(int b) {
  ...
  if ((sliceno = emit_section->numslices) >= emit_section->allocslices) {
    ...
    if ((old_alloced = emit_section->allocslices) != 0)
      emit_section->slices =
          realloc(emit_section->slices, (emit_section->allocslices <<= 1) *
...
void be_emitn(int bi, numtype n) {
  int by = (bi + 7) >> 3;
  int all_by = by;
  if (emit_section->left < by) be_extend(by);
...
int write_section_data(struct section* s, FILE* f) {
  ...
  for (n = 0; n < s->numslices; n++)
    if (fwrite(s->slices[n].base, 1, s->slices[n].used, f) != s->slices[n].used)
      return 0;
  return 1;
}
...
int write_section_srec(struct section* s, FILE* f, char* tmpl) {
  int cs, i, n, l, a = 0, p;
  if (tmpl == NULL) tmpl = "S1%02X%04X";
  for (n = 0; n < s->numslices; n++)
    for (p = 0; p < s->slices[n].used; a += l, p += l) {
      if ((l = s->slices[n].used - p) > 16) l = 16;
      if (fprintf(f, tmpl, l, a) < 0) return 0;
      cs = l + ((a & 0xff000000) >> 24) + ((a & 0xff0000) >> 16) +
           ((a & 0xff00) >> 8) + (a & 0xff);
      for (i = 0; i < l; i++) {
        int b = ((unsigned char*)s->slices[n].base)[p + i];
        cs += b;
        if (fprintf(f, "%02X", b) < 0) return 0;
      }
      if (fprintf(f, "%02X\n", (-cs) & 0xff) < 0) return 0;
    }
  return 1;
...
static void free_section(struct section* s) {
  ...
  for (n = 0; n < s->allocslices; n++)
    if (s->slices[n].allocated) free(s->slices[n].base);

  if (s->allocslices) free(s->slices);
}
...
void backend_finalize() { close_emit_section(); }

void reset_sections() {
  int n;
  reset_section(&builtin_section);
  for (n = 0; n < num_sections; n++) reset_section(&all_sections[n]);
  if (current_loc != NULL) *current_loc = 0;
}
