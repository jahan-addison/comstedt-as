...
static int make_id(const char* filename, int is_macro) {
  int i;
  for (i = 0; i < numids; i++)
    if (allfiles[i].filename == filename && allfiles[i].is_macro == is_macro)
      return i;
  if (numids >= numfilebriefs)
    if (allfiles)
      allfiles =
          realloc(allfiles, sizeof(struct file_brief) * (numfilebriefs <<= 1));
    else
...
void filestack_push(int id) {
  ...
  if (filestackdepth) filestack[filestackdepth - 1].lineno = current_lineno;
...
void process_buffer(const char* filename, int lineno,
  ...
  if (filestackdepth >= numfileinfos)
    if (filestack)
      filestack =
          realloc(filestack, sizeof(struct file_info) * (numfileinfos <<= 1));
    else
  ...
  if ((filestack[filestackdepth++].file = f)) yyrestart(f);
...
void add_incdir(const char* name) {
  ...
  if ((l = strlen(name)) > maxincdirlen) maxincdirlen = l;
}
