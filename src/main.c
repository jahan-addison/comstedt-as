...
int main(int argc, char* argv[]) {
  ...
  smach_init();
  backend_init();
  while ((c = getopt(argc, argv, "hxVI:")) != EOF) switch (c) {
      case 'h':
  ...
  for (i = optind; i < argc; i++) {
    ...
    if (process_file(argv[i])) {
      ...
      if (undecided != NULL && !passes) {
        fprintf(stderr,
                "%s line %d: can't find invariant for %s after %d passes\n",
      ...
      symbol_end();
      fprintf(stderr,
              "Assembly of %s completed in %d ms, %d lines munched, "
              "total errors %d\n",
      ...
      if (numerrors)
        ...
        if ((f = fopen(fn, "wb")) != NULL) {
          ...
          if (hexout) fprintf(f, ":00000001FF\n");
          fclose(f);
        } else {
          perror(fn);
          rc = 1;
