...
#define ALLOT(b, n)                                                \
  ((emit_p + (n) + 1 < emit_brk ? (void)0 : extend_emit((n) + 1)), \
   *emit_p++ = (b), (emit_p += (n)))
  ...
  if (SMVelt(v, type) == M_ICON) {
    switch (t) {
      ...
      case M_CHECKW:
        ...
        break;
      case M_CHECKX:
        if (SMVelt(v, number.num) < -(1 << b) ||
            SMVelt(v, number.num) >= (1 << b)) {
...
static void extend_emit(int n) {
  if (emit_p) *emit_p = OP_BREAK;
  if (++n < CODEBLKSZ) n = CODEBLKSZ;
  emit_p = malloc(n);
  emit_brk = emit_p + n;
  if (emit_p == NULL) {
    fprintf(stderr, "Fatal: Out of memory!\n");
...
void smach_dump(FILE* f, SMV v) {
  switch (SMVelt(v, type)) {
    case M_ICON:
      fprintf(f, "%ld", SMVelt(v, number).num);
      break;
    case M_SYMB:
      fprintf(f, "%s", SMVelt(v, symbol).symb->name);
      break;
    case M_NEG:
      fprintf(f, "-");
      smach_dump(f, SMVelt(v, unary).v);
      break;
    case M_CPL:
      fprintf(f, "~");
      smach_dump(f, SMVelt(v, unary).v);
    ...
    case M_LS1:
    case M_LS2:
      smach_dump(f, SMVelt(v, ls).v);
      fprintf(f, "<<%d", SMVelt(v, ls).n);
      break;
    case M_CHECKU:
      fprintf(f, "U%d(", SMVelt(v, ls).n);
      smach_dump(f, SMVelt(v, ls).v);
      fprintf(f, ")");
      break;
    case M_CHECKS:
      fprintf(f, "S%d(", SMVelt(v, ls).n);
      smach_dump(f, SMVelt(v, ls).v);
      fprintf(f, ")");
      break;
    case M_CHECKW:
      fprintf(f, "W%d(", SMVelt(v, ls).n);
      smach_dump(f, SMVelt(v, ls).v);
      fprintf(f, ")");
      break;
    case M_CHECKX:
      fprintf(f, "X%d(", SMVelt(v, ls).n);
      smach_dump(f, SMVelt(v, ls).v);
      fprintf(f, ")");
      break;
    default:
...
static void emit_prefix(numtype n) {
  ...
  EMIT(OP_PREFIX(n));
  while (z--) EMIT(OP_PREFIX(s[z]));
}

static void emit_symprefix(struct symbol* s) { emit_prefix((numtype)(void*)s); }
...
static struct symbol* locate_sym(numtype n) { return (struct symbol*)(void*)n; }
    ...
    */
    if (o & 0x80) {
      if (o & 0x40) switch (o) {
          ...
          case OP_ORG: {
            numtype a = POP;
            while (*current_loc < a) EMIT8(0);
            break;
          }
          case OP_CNOP: {
            numtype b = findmsb(pfx);
            pfx &= (b - 1);
            while (((*current_loc) & (b - 1)) != pfx) EMIT8(0);
            pfx = 0;
            break;
          }
          case OP_LINENO:
            current_lineno += pfx + 1;
            pfx = 0;
            break;
          case OP_FILENAME:
            if (pfx) {
              filestack_push(pfx - 1);
              current_lineno = 0;
              pfx = 0;
            } else
              filestack_pop();
...
struct symbol* smach_run() {
  ...
  current_lineno = 0;
  reset_sections();
  for (p = 0; p < regd_blks; p++) pfx = smach_execute(code_blk[p], pfx);
  return changed;
}
...
void smach_end() {
  if (code_blk != NULL) {
    int i;
    for (i = 0; i < regd_blks; i++) free(code_blk[i]);
    free(code_blk);
  }
  if (SMVs) free(SMVs);
}
